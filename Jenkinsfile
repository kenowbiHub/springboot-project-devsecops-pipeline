...
pipeline {
  agent { label 'build' }
   environment { 
        registry = "kingslifydockerhub/democicd" 
  ...
  stages {
   ...
   stage('Stage IV: SAST') {
      steps { 
        ...
        withSonarQubeEnv('mysonarqube') {
            sh 'mvn sonar:sonar -Dsonar.coverage.jacoco.xmlReportPaths=target/site/jacoco/jacoco.xml -Dsonar.dependencyCheck.jsonReportPath=target/dependency-check-report.json -Dsonar.dependencyCheck.htmlReportPath=target/dependency-check-report.html -Dsonar.projectName=onesolution'
       }
      }
    }
   ...
   stage('Stage VII: Scan Image ') {
      steps { 
        ...
        sh "trivy image --scanners vuln --offline-scan kingslifydockerhub/democicd:latest > trivyresults.txt"
        }
    }
